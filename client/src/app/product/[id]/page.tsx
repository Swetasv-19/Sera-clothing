  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        <Icon icon="mdi:alert-circle-outline" className="text-[var(--muted)] w-16 h-16" />
        <h2 className="text-2xl font-serif font-bold text-[var(--foreground)]">Product not found</h2>
              ...
              {product.images.map((img, i) => (
                <button 
                  ...
                  className={`w-24 aspect-[4/5] rounded-lg overflow-hidden border-2 transition-all flex-shrink-0 ${
                    activeImage === i ? "border-[var(--accent-primary)] scale-95" : "border-transparent opacity-70 hover:opacity-100"
                  }`}
                >
        ...
        <div className="flex flex-col">
          <div className="mb-6">
            ...
            <div className="flex items-center gap-4 mb-4">
              <div className="flex text-yellow-500">
                {[...Array(5)].map((_, i) => (
                  <Icon key={i} icon={i < 4 ? "mdi:star" : "mdi:star-outline"} width={20} />
                ))}
              </div>
              <span className="text-sm text-[var(--muted)] font-medium">(120 Reviews)</span>
            </div>
            ...
            {product.colors.length > 0 && (
              <div>
                <h3 className="text-sm font-bold uppercase tracking-wider text-[var(--foreground)] mb-4">
                  Select Color: <span className="text-[var(--muted)] normal-case font-medium">{selectedColor}</span>
                </h3>
          ...
          <div className="mt-10 pt-10 border-t border-[var(--divider)] space-y-4">
            <div className="flex items-center gap-3 text-sm text-[var(--muted)]">
              <Icon icon="mdi:truck-fast-outline" className="w-5 h-5 text-[var(--accent-primary)]" />
              <span>Complimentary shipping on orders over $150</span>
            </div>
            <div className="flex items-center gap-3 text-sm text-[var(--muted)]">
              <Icon icon="mdi:refresh" className="w-5 h-5 text-[var(--accent-primary)]" />
